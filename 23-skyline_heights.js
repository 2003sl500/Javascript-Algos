...
// Page 53, Skyline Heights


let buildingSize = [1, -1, 7, 3]
let visible = [];

for(let i = 0; i < buildingSize.length; i++){
    if(i === 0 && buildingSize[i] > 0){
        visible.push(buildingSize[0]);
    } else if((buildingSize[i] > buildingSize[i - 1]) && buildingSize[i] > 0){
        visible.push(buildingSize[i]);
    }
}

console.log("Building sizes visible: " + visible);